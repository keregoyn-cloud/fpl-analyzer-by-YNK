...
def best_starting_xi(squad: pd.DataFrame) -> Tuple[pd.DataFrame, str]:
    """Choose the highest-scoring legal formation from the 15-man squad."""
    best_xi = None
    best_score = float("-inf")
    best_formation = ""

    # FPL allows 1 GK, >=3 DEF, >=2 MID, >=1 FWD.
    for defenders in range(3, 6):
        for midfielders in range(2, 6):
            for forwards in range(1, 4):
                ...
                gks = squad[
                    squad["position"] == "Goalkeeper"
                ].sort_values("gw_projection", ascending=False).head(1)

                defs = squad[
                    squad["position"] == "Defender"
                ].sort_values(
                    "gw_projection",
                    ascending=False,
                ).head(defenders)
                ...
                if (
                    len(gks) != 1
                    or len(defs) != defenders
                    or len(mids) != midfielders
                    or len(fwds) != forwards
                ):
                    continue
                ...
                score = float(xi["gw_projection"].sum())
...
def main():
    ...
    st.caption(
        "Gameweek-by-Gameweek FPL optimizer using official FPL data, "
        "fixture difficulty and optional Google News signals."
    )
    ...
    with tabs[2]:
        ...
        st.write(
            "- Use the best legal starting formation rather than forcing 3-4-3."
        )
        st.write(
